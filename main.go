...
func cmdQuickConnect(opts docopt.Opts) {
    ...
    if bg, _ := opts.Bool("--background"); bg {
        pid, err := spawnBackground(os.Args)
        if err != nil { fatal(fmt.Errorf("background start failed: %w", err)) }
        fmt.Printf("started in background pid=%d\n", pid)
        return
    }
    ...
    if logPath != "" { if err := setupLogFile(logPath); err != nil { fatal(fmt.Errorf("log setup failed: %w", err)) } }
    lvl := strings.TrimSpace(getStringOr(opts, "--log_level", ""))
    dbg, _ := opts.Bool("--debug")
    setLogLevel(lvl, dbg)
    ...
    if userAuth != "" || password != "" {
        if userAuth == "" || password == "" { fatal(errors.New("--user_auth and --password must be provided together")) }
        ...
        api.AuthLoginWithPassword(&connect.AuthLoginWithPasswordArgs{UserAuth: userAuth, Password: password}, connect.NewApiCallback(func(res *connect.AuthLoginWithPasswordResult, err error) {
            defer close(done)
            if err != nil { logError("login error: %v\n", err); return }
            if res.Error != nil { logError("login error: %s\n", res.Error.Message); return }
        ...
        if codeOpt != "" {
            ...
            api2.AuthVerify(&connect.AuthVerifyArgs{UserAuth: userAuth, VerifyCode: codeOpt}, connect.NewApiCallback(func(res *connect.AuthVerifyResult, err error) {
                defer close(done2)
                if err != nil { logError("verify error: %v\n", err); return }
                if res.Error != nil { logError("verify error: %s\n", res.Error.Message); return }
                if res.Network == nil || strings.TrimSpace(res.Network.ByJwt) == "" { logError("verify succeeded but no by_jwt returned\n"); return }
                if err := saveJWT(res.Network.ByJwt); err != nil { logError("save jwt failed: %v\n", err); return }
    ...
    var clientIDStr string
    {
        ...
        if err != nil { fatal(errors.New("no JWT available; provide --user_auth/--password to login or --jwt to use an existing token")) }
        ...
        if id := parseClientID(jwt); id != "" && !forceJWT {
            if validateClientJWT(apiUrl, jwt) {
                ...
                if retryEvery <= 0 { retryEvery = time.Minute }
                for {
                    refreshed := false
                    ...
                    if userAuth != "" && password != "" {
                        ...
                        api2.AuthLoginWithPassword(&connect.AuthLoginWithPasswordArgs{UserAuth: userAuth, Password: password}, connect.NewApiCallback(func(lr *connect.AuthLoginWithPasswordResult, err error) {
                            ...
                            if err != nil || lr == nil || lr.Error != nil || lr.Network == nil || strings.TrimSpace(lr.Network.ByJwt) == "" {
                                if err != nil { logWarn("jwt refresh: login failed: %v\n", err) } else if lr != nil && lr.Error != nil { logWarn("jwt refresh: login failed: %s\n", lr.Error.Message) } else { logWarn("jwt refresh: login failed\n") }
                                return
                            }
                            ...
                            if mres, merr := api2.AuthNetworkClientSync(&connect.AuthNetworkClientArgs{Description: "", DeviceSpec: ""}); merr == nil && mres != nil && mres.Error == nil && strings.TrimSpace(mres.ByClientJwt) != "" {
                                if err := saveJWT(mres.ByClientJwt); err != nil {
                                    ...
                                    if validateClientJWT(apiUrl, newJWT) {
                                        if id2 := parseClientID(newJWT); id2 != "" { clientIDStr = id2 }
            ...
            if res.Error != nil { fatal(fmt.Errorf("auth-client error: %s", res.Error.Message)) }
            if strings.TrimSpace(res.ByClientJwt) == "" { fatal(errors.New("auth-client succeeded but no by_client_jwt returned")) }
            if err := saveJWT(res.ByClientJwt); err != nil { fatal(err) }
            clientIDStr = parseClientID(res.ByClientJwt)
            if clientIDStr != "" { logInfo("saved client JWT (client_id=%s) -> %s\n", clientIDStr, jwtPath()) } else { logInfo("saved client JWT -> %s\n", jwtPath()) }
        }
    }
                ...
                case <-ticker.C:
                    ...
                    if userAuth != "" && password != "" {
                        ...
                        api2.AuthLoginWithPassword(&connect.AuthLoginWithPasswordArgs{UserAuth: userAuth, Password: password}, connect.NewApiCallback(func(lr *connect.AuthLoginWithPasswordResult, err error) {
                            defer close(done)
                            if err != nil || lr == nil || lr.Error != nil || lr.Network == nil || strings.TrimSpace(lr.Network.ByJwt) == "" {
                                if err != nil { logWarn("jwt renew: login failed: %v\n", err) } else if lr != nil && lr.Error != nil { logWarn("jwt renew: login failed: %s\n", lr.Error.Message) } else { logWarn("jwt renew: login failed\n") }
                                return
                            }
                            byJwt = lr.Network.ByJwt
                        }))
                        ...
                        if strings.TrimSpace(byJwt) != "" {
                            ...
                            if mres, merr := api2.AuthNetworkClientSync(&connect.AuthNetworkClientArgs{Description: "", DeviceSpec: ""}); merr == nil && mres != nil && mres.Error == nil && strings.TrimSpace(mres.ByClientJwt) != "" {
                                if err := saveJWT(mres.ByClientJwt); err != nil {
                                    ...
                                    if id := parseClientID(mres.ByClientJwt); id != "" { logInfo("jwt renewed (client_id=%s)\n", id) } else { logInfo("jwt renewed\n") }
                                }
                            }
                        }
                        cancel2()
...
func buildVpnArgsFromOpts(opts docopt.Opts, apiUrl, connectUrl string) []string {
    argv := []string{"vpn"}
    add := func(flag, val string) { val = strings.TrimSpace(val); if val != "" { argv = append(argv, fmt.Sprintf("%s=%s", flag, val)) } }
    addBool := func(flag string, on bool) { if on { argv = append(argv, flag) } }
    // Core endpoints
    add("--api_url", apiUrl)
    ...
    if mtu := getIntOr(opts, "--mtu", 0); mtu > 0 { add("--mtu", fmt.Sprintf("%d", mtu)) }
    ...
    if si := getIntOr(opts, "--stats_interval", -1); si >= 0 { add("--stats_interval", fmt.Sprintf("%d", si)) }
    if dbg, _ := opts.Bool("--debug"); dbg { argv = append(argv, "--debug") }
    return argv
}
...
func spawnProcessDetached(args []string, logPath string) (int, error) {
    ...
    if strings.TrimSpace(logPath) != "" {
        // ensure dir exists
        if dir := filepath.Dir(logPath); dir != "." && dir != "" { _ = os.MkdirAll(dir, 0o755) }
...
func cmdLocations(opts docopt.Opts) {
    ...
    if res == nil { fmt.Println("no results"); return }
    ...
    if len(res.Locations) > 0 {
        ...
        for _, l := range res.Locations {
            ...
            if l.CountryLocationId != "" { id = l.CountryLocationId }
            if l.RegionLocationId != "" { id = l.RegionLocationId }
            extra := l.Country
            if l.Region != "" { extra = l.Region }
            if extra != "" {
...
func cmdOpen(opts docopt.Opts) {
    ...
    if clientIDStr == "" { fatal(errors.New("JWT missing client_id (run 'urnet-client mint-client' to mint a client-scoped JWT)")) }
...
// helpers
func getStringOr(opts docopt.Opts, key, def string) string { if v, err := opts.String(key); err == nil && v != "" { return v }; return def }
func getIntOr(opts docopt.Opts, key string, def int) int { if v, err := opts.Int(key); err == nil { return v }; return def }
func mustString(opts docopt.Opts, key string) string { v, _ := opts.String(key); if strings.TrimSpace(v) == "" { fmt.Fprintf(os.Stderr, "%s is required\n", key); os.Exit(2) }; return v }
...
func idsToStrings(ids []connect.Id) []string {
    ...
    for _, id := range ids { out = append(out, id.String()) }
    return out
}
...
func spawnBackground(argv []string) (int, error) {
    ...
    for i, a := range argv {
        ...
        if a == "--background" || strings.HasPrefix(a, "--background=") { continue }
        args = append(args, a)
    }
    cmd := exec.Command(exe, args...)
...
func findSpecsByQueryFallback(ctx context.Context, strat *connect.ClientStrategy, apiUrl, jwt, q string) []*connect.ProviderSpec {
    ...
    if !ok { key = "name"; val = strings.TrimSpace(q) }
...
func filterLocationsFallback(ctx context.Context, strat *connect.ClientStrategy, apiUrl, jwt, q string) ([]*connect.ProviderSpec, *findLocationsHTTPResult) {
    ...
    if err != nil || res == nil { return nil, nil }
    key, val, ok := parseKV(q)
    if !ok { key = "name"; val = strings.TrimSpace(q) }
    key = strings.ToLower(strings.TrimSpace(key))
    valNorm := strings.ToLower(strings.TrimSpace(val))
    ...
    for _, g := range out.Groups {
        if id, err := connect.ParseId(g.LocationGroupId); err == nil {
            if !seen[id.String()] { seen[id.String()] = true; specs = append(specs, &connect.ProviderSpec{LocationGroupId: &id}) }
        }
    }
    for _, l := range out.Locations {
        idStr := l.LocationId
        if key == "country" && l.CountryLocationId != "" { idStr = l.CountryLocationId }
        if key == "region" && l.RegionLocationId != "" { idStr = l.RegionLocationId }
        if id, err := connect.ParseId(idStr); err == nil {
            if !seen[id.String()] { seen[id.String()] = true; specs = append(specs, &connect.ProviderSpec{LocationId: &id}) }
        }
    }
    return specs, out
}
